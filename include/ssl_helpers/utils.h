...
std::time_t from_iso_string(const std::string& formatted, const char* time_format, bool should_utc = true);

// Convert UNIX time to readable ISO-datetime

std::string to_iso_string(const std::time_t, bool should_utc = true);
std::string to_iso_string(const std::time_t, const char* time_format, bool should_utc = true);
